...
sc_tbl = as.data.frame(sc_tbl)

sc_tbl_edit = sc_tbl %>% 
  filter(State != "District of Columbia" & State != "Rhode Island")

sc_tbl_edit = as.tibble(sc_tbl_edit)

sc_out_edit = sc_tbl_edit %>%
  # initial the synthetic control object
  synthetic_control(outcome = Mort_rate,
                    unit = State, 
                    time = Year,
...
save(sc_out_edit, file = "r_objects/sc_out_edit.Rdata")
